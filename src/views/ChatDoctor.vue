<template>
  <section>
    <div style="overscroll-behavior: none">
      <!-- HEADING -->
      <div
        class="
          fixed
          w-full
          bg-primary
          h-28
          pt-2
          text-grey-400
          flex
          justify-between
          shadow-md
        "
        style="top: 0px; overscroll-behavior: none"
      >
        <!-- back button -->
        <router-link class="mt-6" :to="{ name: 'Hospital' }">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            class="w-12 h-12 my-1 text-gray-100 ml-2"
          >
            <path
              class="text-gray-100 fill-current"
              d="M9.41 11H17a1 1 0 0 1 0 2H9.41l2.3 2.3a1 1 0 1 1-1.42 1.4l-4-4a1 1 0 0 1 0-1.4l4-4a1 1 0 0 1 1.42 1.4L9.4 11z"
            />
          </svg>
        </router-link>
        <div
          class="my-3 mt-10 text-black font-bold text-lg tracking-wide"
          v-if="user.role == 'Customer'"
        >
          Hello {{ user.email }}!, Need help?
        </div>
        <div
          class="my-3 text-gray-100 font-bold text-lg tracking-wide"
          v-if="user.role == 'Admin'"
        >
          Customers
        </div>
        <!-- 3 dots -->
        <div class="mt-5">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            class="icon-dots-vertical w-8 h-8 mt-2 mr-2"
          >
            <path
              class="text-green-100 fill-current"
              fill-rule="evenodd"
              d="M12 7a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 7a2 2 0 1 1 0-4 2 2 0 0 1 0 4zm0 7a2 2 0 1 1 0-4 2 2 0 0 1 0 4z"
            />
          </svg>
        </div>
      </div>

      <!-- MESSAGES -->
      <div class="mt-20 mb-16">
        <div class="clearfix">
          <div class="bg-gray-300 w-3/4 mx-4 my-2 p-2 rounded-lg">
            <!-- <div class="msg_history"> -->
            <IncomingMsg
              v-for="(msg, index) in messages"
              :key="index"
              :message="msg"
            ></IncomingMsg>
            <!-- </div> -->
          </div>
        </div>
      </div>
    </div>

    <!-- MESSAGE INPUT AREA -->
    <div
      class="fixed w-full flex justify-between bg-gray-400"
      style="bottom: 0px"
    >
      <input
        type="text"
        v-model="inputMsg"
        @keyup.enter="sendMessage"
        class="
          flex-grow
          m-2
          py-2
          px-4
          mr-1
          rounded-full
          border border-gray-300
          bg-gray-200
          resize-none
        "
        rows="1"
        placeholder="Type a message"
        style="outline: none"
      />
      <button
        class="m-2"
        style="outline: none"
        type="button"
        @click.prevent="sendMessage"
      >
        <svg
          class="
            svg-inline--fa
            text-gray-600
            fa-paper-plane fa-w-16
            w-12
            h-12
            py-2
            mr-2
          "
          aria-hidden="true"
          focusable="false"
          data-prefix="fas"
          data-icon="paper-plane"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 512 512"
        >
          <path
            fill="currentColor"
            d="M476 3.2L12.5 270.6c-18.1 10.4-15.8 35.6 2.2 43.2L121 358.4l287.3-253.2c5.5-4.9 13.3 2.6 8.6 8.3L176 407v80.5c0 23.6 28.5 32.9 42.5 15.8L282 426l124.6 52.2c14.2 6 30.4-2.9 33-18.2l72-432C515 7.8 493.3-6.8 476 3.2z"
          />
        </svg>
      </button>
    </div>
  </section>
</template>

<script>
import IncomingMsg from "../components/IncomingMsg";
import { mapActions, mapMutations, mapState } from "vuex";
export default {
  name: "HelpCenter",
  data: function() {
    return {
      inputMsg: "",
    };
  },
  computed: {
    ...mapState(["user", 'messages'])
  },
  sockets: {
    broadcastMessage(data) {
      // console.log(data, "sockets client");
      this.pushMessage(data)
    }, //sesuai yang ada di io.emit di app.js server
  },
  methods: {
    ...mapActions(["findUserLogin"]),
    ...mapMutations({
      pushMessage: 'PUSH_MESSAGE'
    }),
    async sendMessage() {
      const data = {
        username: this.user.username,
        message: this.inputMsg,
      };
      await this.pushMessage(data)
      await this.$socket.client.emit("sendMessage", data);
      this.inputMsg = "";
    }
  },
  async created() {
    await this.findUserLogin()
  },
  components: {
    IncomingMsg,
  },
};
</script>
